...
print 'The customer has been succesfully deleted.'\end{lstlisting}
	 ...
	 \section{cursors}
	 SQL statements are processing complete resultsets and not individual rows. Cursors allow to process individual rows to preform complex, row specifik operations that can't (easily be preformed with a single SELECT, UPDATE or DELETE statement. It is a datbase object that refers to the result of a query. It allows to specify the row from the resultset you wish to process.
...
OPEN <cursor_name>\end{lstlisting}
		\item \textbf{FETCH} : fetches 1 row
		\item \textbf{CLOSE} : closes the cursor (counterpart to OPEN)
...
\end{document}
>>>>>>> 594aa8ffe7ac1c0ef238e1e089c7e455bc917ed8
